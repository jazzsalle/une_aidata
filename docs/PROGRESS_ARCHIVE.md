# PROGRESS_ARCHIVE.md — 완료 기록 보관

`PROGRESS.md` 가 36,967자까지 자라 SessionStart 훅 주입 한도(10,000자)를 크게 넘겼다.
그래서 **완료된 작업 기록을 이 파일로 옮기고**, `PROGRESS.md` 에는 "지금 무엇을 해야 하는지"만 남겼다(2026-08-09).

- 아래 내용은 옮긴 시점의 원문 그대로다. 요약하거나 고치지 않았다.
- 현재 상태·다음 할 일·Blocker 는 `PROGRESS.md` 를 본다.
- 완료 이력을 찾을 때만 이 파일을 연다.

---

## [2026-08-25 이관] Last updated (08-23 저녁 상세)
...
## [2026-08-23 이관] Last updated (08-21 저녁 상세)
...
## [2026-08-21 이관] Last updated 상세 (08-21 낮 · 08-19)
2026-08-21 — **T3Q 재난메타 반영 P1~P4 전부 머지(PR #34~#37).** 표본 3지역(대구 서구·정읍·
김해) 비교본, Agent CQ 문답+지도 바로가기, 하단 패널 3개(계획지식·사건타임라인·위성 미리보기),
불필요 항목 3건 제거(flood-traces API·river-zone 소스·Mock 검색 패널 UI). 구분 규약: id 의
`-META-` · data_status 'meta_demo' · 구분색 .meta-demo-text — 실데이터 오면 이 규약 행만 걷는다.
표본 시드 재생성은 `npm run data:meta-demo`(회사 PC — GIS_data 필요). 상세는 docs/34 반영 결과 절.

앞서 2026-08-19: 하천 전국화·침수흔적도 등 PR #21~#33 (자세한 건 각 PR 본문):
- **#21** 하천 자료 전국 229개 시군구 + 시군구 코드 축 통일(`sgg_code_map` `primary_code`)
- **#22** VWorld 배포 설정 절차 문서(docs/16 §2.1~2.2) — 프로덕션 배경지도는 원래 정상이었고
  내가 네트워크를 너무 일찍 읽어 오진했던 것. 판정 기준을 상태칩으로 적어 둠
- **#23** 시군구 경계 230곳(행정동 union) · 지역 선택 시 경계에 fit · 소하천 스위치 ·
  **소하천명 별도 레이어**(초록)
- **#24** 행안부 침수흔적도 **37,987건** 전처리 반입(`reference/flood/`) · 코드표가 종전 코드를
  이름으로 잇게 보강(코드미상 4,964 → 16)
- **#25~26** 하천 검색이 시군구로 거르고 · 참조 자료 URL 에 빌드 토큰(`?v=<sha8>`) · 경계 누적 제거 ·
  검색 초기화(✕·Esc)
- **#27** POC mock 레이어 넷(침수흔적 seed·홍수위험·위험저수지·풍수해개선)과 VWorld WMS 직결을
  **지도에서만** 뺌 — 시드·API·OpenAPI 계약은 그대로(승인 대기 항목)
- **#28** 침수흔적 주황 · **#29** 국가·지방하천 검색은 고른 시군구 안의 조각으로 fit ·
  **#30** 다른 지역 하천을 고르면 그 지역으로 옮김(상단까지) + 경계 자동 켜기 ·
  **#31** 시군구 배정을 교차 판정으로(한강 19→24) + 지나는 시군구 칩으로 펼쳐 골라 감
...
## Done this session
- **소하천구역·전국하천표준데이터 반입 + 지도 6개 지역 확대 (2026-08-14)**: 사용자가 리포 루트에 올린 두 원자료(`소하천_소하천구역(연속주제)+브이월드/`, `전국하천표준데이터/`)를 분석해 구미·의왕·남원·부산·인제·영천만 잘라 반입했다. 백로그였던 "지역 선택을 검색창으로" 건을 함께 해소했다.
  - **소하천구역(LSMD_CONT_UJ301)**: 시도별 SHP → `COL_ADM_SE`(시군구코드) 정확 필터(bbox 자르기 아님 — 인접 시군 형상이 섞인다) → EPSG:5186→4326 → 2 m 단순화. 산출 **1,531건**(의왕 110·구미 159·남원 454·영천 115·인제 643·부산 50, 2.5 MB). 좌표계는 같은 배포본의 `_5174_`(구 측지계 Korean 1985)가 아니라 `Korea 2000 / Central Belt 2010`(EPSG:5186)을 썼다 — 데이텀 변환 단계가 하나 줄어든다. 5개 시도 표본 변환으로 착지 실측(기장 129.222/35.272, 의왕 126.956/37.317, 구미 128.175/36.255, 남원 127.419/35.435, 인제 128.287/38.040).
  - **하천명이 들어 있는 칸이 지자체마다 다르다**: 영천·부산은 `ALIAS`, 구미·남원·의왕은 `ALIAS`가 '소하천구역' 같은 일반값이고 실제 이름은 `REMARK`에, 인제는 `'3-01 소재골천 소하천구역'`처럼 고시번호+이름+구분이 한 칸에 있다. 셋 다에서 읽되 **못 읽으면 `stream_name`을 붙이지 않는다**(원문은 `alias_raw`/`remark_raw` 보존). 판독 결과 의왕 25·구미 155·남원 301·영천 112·인제 133·부산 23종.
  - **전국하천표준데이터 마커**: 원자료 2,558건 중 **위경도 보유는 194건뿐**이고 6개 지역으로 좁히면 부산 17 + 의왕 1 = **18건(시점·종점 마커 36개)**. 남원 352건·영천 25건은 전부 좌표 없음, 구미·인제는 레코드 자체가 0건. 사용자 결정(2026-08-14)에 따라 **좌표를 실제로 가진 건만 마커**로 만들고 나머지 409건은 `river_standard_catalog.json`에 `has_coordinate:false`로 남겨 검색 목록에서만 보인다. **주소를 지오코딩해 좌표를 만들지 않는다** — 만들면 실측 좌표와 파생 좌표가 화면에서 구분되지 않고 `official_data=true` 표기가 사실이 아니게 된다.
  - **지도 전용 지역 선택기**: 부산·인제·영천은 위험지구·우선 확인지역 시드가 없어 앱 지역(`adminCode`)이 될 수 없다(대시보드 '현재 판단' 목록이 통째로 빈다). 앱 지역 계약·`current_situations_seed`는 그대로 두고 `apps/web/src/features/map/mapRegions.ts` + 검색 패널의 선택기로 **지도만** 6곳을 오간다. 앱 지역이 바뀌면 지도도 따라간다. 남원은 앱 코드 45190 ↔ 공간자료 코드 52190(전북특별자치도 개편)이라 `scripts/river_regions.py`에서 두 코드를 나눠 든다(`data/reference/admin_code_map.json`과 같은 방향의 판단이다).
  - **마우스 오버 텍스트 태그**: 어댑터에 `onFeatureHover` 신설 — 캔버스 벡터는 DOM 이 아니라 마우스 이벤트를 직접 받지 못한다. 커서 옆에 '이름 + 자료구분 + 한 줄'만 띄우고 표는 클릭 팝업이 맡는다. 같은 피처 위 미세 이동에는 재발행하지 않는다(태그 떨림). 지도 캔버스가 `aria-hidden`이라 태그도 그렇게 뒀고, 같은 내용은 검색 목록·클릭 팝업이 접근 가능 경로로 제공한다.
  - **하천명 검색**: 지역당 최대 3.5 MB GeoJSON 을 검색마다 받지 않도록 전처리에서 `river_search_index.json`(1,835건·424 KB)을 만들고 **검색을 처음 열 때 한 번만** 받는다. 결과 클릭 시 지역 전환 → 소스 켜기 → 좌표 이동 → 자료 도착 후 형상 강조. `nav_kind`로 `actual`(원자료 좌표)·`extent`(형상 bbox 중심 = 화면이동 전용)·`none`(좌표 없음, 이동 비활성)을 구분한다.
  - 신규: `scripts/river_regions.py`(지역 카탈로그 단일 출처) · `build_sochun_layers.py` · `build_river_standard_points.py` · `build_river_search_index.py` · `smoke_river_reference_layers.py`. `npm run data:rivers`로 3종 재생성. 원자료 폴더 2개(약 460 MB)는 `.gitignore` 추가. `requirements.txt`에 numpy·pyproj·pyshp 누락분 보강.
  - 검증: typecheck(web·functions), build, `validate_vercel_repo`, seed·priority·mock-spatial·a11y 스모크, OpenAPI 30/30, JSON Schema 265/18, 신규 `smoke_river_reference_layers` 전부 PASS. 브라우저 실측 — 영천 소하천 115건, 부산 마커 34개 + 팝업(호계천 시점 35.14054/129.04658 · `value_status=actual`), 인제 643건, 검색→지역전환→강조, console error 0.
  - **미해결**: 부산·인제·영천의 국가기본도 하천 3종은 원본 SHP 가 없어 추출하지 못했다(레이어 메뉴에 '이 지역 자료 없음'). 원본 수령 후 `extract_river_layers.py`의 대상지역 산정을 `geo.json` L3 bbox 대신 `river_regions.py`로 바꿔 6곳 전부 재추출한다.
