...
**설정·변경 후에는 재배포해야 반영된다.** `VITE_*` 는 Vite 가 **빌드 시점에 번들에 박는** 값이고
런타임 조회가 아니다. 환경변수만 넣고 재배포하지 않으면 이미 배포된 빌드는 그대로다. main 머지처럼
새 빌드가 도는 일이 예정돼 있으면 **환경변수를 그 전에** 넣어 빌드를 한 번으로 끝낸다.

키가 실제로 주입됐는지는 배포된 번들로 확인할 수 있다. 키가 없는 빌드는 `VWorldMapAdapter.ts` 의
`if (key)` 가 거짓으로 접혀 **else 분기 문구(`VWorld 키 미설정: 공간 Seed만 표시`)만 남고 WMTS
URL 이 사라진다.** 반대로 키가 있으면 그 문구가 번들에서 사라진다. 키 값을 꺼내 볼 필요 없이
**문구의 유무로** 판정하면 된다.
...
1. 배포된 주소를 브라우저로 연다.
2. **타일이 붙을 때까지 기다린다.** 지도 초기화와 타일 요청 사이에 시차가 있어서, 첫 페인트
   직후에는 배경이 비어 있고 상태칩이 `VWorld 배경지도 연결 중` 에 머문다. 2026-08-19 에 이
   시차를 못 보고 "요청 0건 → 키 미주입" 으로 오판했다. **요청 건수 0 은 아직 안 왔다는 뜻일
   수 있다.** 상태칩이 `연결 중` 이면 판정을 미룬다.
3. 네트워크에 `api.vworld.kr/req/wmts/1.0.0/{key}/Base/{z}/{y}/{x}.png` 요청이 **200** 으로
   뜨는지 본다.
4. 지도 좌상단 상태칩이 `VWorld 일반지도 연결 정상` 인지 본다. 이 문구는 `tileloadend` 에서만
   갱신되므로 **타일이 실제로 하나 이상 그려졌다는 증거**이고, 그래서 `verified` 승격의 근거가
   된다.
5. 타일이 거절되면 키가 아니라 **등록 도메인부터** 본다. 이때 상태칩은
   `VWorld ... 로딩 실패: 키·등록도메인을 확인하세요` 다. 상태별로 원인이 갈린다.

| 상태칩 | 뜻 | 볼 곳 |
|---|---|---|
| `연결 중` 에서 안 바뀜 | 아직 타일 응답 전이거나 요청이 안 나갔다 | 더 기다려 본다. 계속이면 뷰 크기·레이어 구성 |
| `연결 정상` | 타일이 그려졌다 | 정상. `verified` 승격 가능 |
| `로딩 실패` | 요청은 갔고 거절됐다 | 등록 도메인 → 키 권한 순 |
| `키 미설정` | 빌드에 키가 없다 | Vercel 환경변수 + 재배포 |
