...
const t3qGateway_js_1 = require("../../../providers/t3qGateway.js");
async function POST(request) { try {
    const input = await (0, http_js_1.body)(request);
    const query = input.query?.trim();
    if (!query)
        return (0, http_js_1.badRequest)('query는 필수입니다.');
    ...
    const result = await (0, t3qGateway_js_1.searchT3qPreview)(payload);
    return (0, http_js_1.envelope)(result, { provider: 'T3qStructureMockProvider', dataStatus: 'mock', warnings: result.warnings });
}
catch (error) {
    return (0, http_js_1.badRequest)(error instanceof Error ? error.message : 'T3Q 검색 미리보기 실패');
} }
