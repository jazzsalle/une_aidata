...
exports.normalizeAgentContext = normalizeAgentContext;
...
const CONTEXT_KINDS = new Set(['district', 'similar_event', 'river']);
const MAX_CONTEXT_ITEMS = 10;
const MAX_EVIDENCE_ITEMS = 10;
const str = (value) => (typeof value === 'string' ? value : '');
const num = (value) => (typeof value === 'number' && Number.isFinite(value) ? value : null);
const rec = (value) => (value && typeof value === 'object' && !Array.isArray(value) ? value : {});
const recs = (value) => (Array.isArray(value) ? value.filter((item) => item && typeof item === 'object').map((item) => item) : []);
const strs = (value) => (Array.isArray(value) ? value.map(String) : []);
const firstPage = (value) => (Array.isArray(value) ? num(value[0]) : num(value));
const fmt = (value) => (value === null ? '미기재' : value.toLocaleString('ko-KR'));
const dateOnly = (value) => (value.length >= 10 ? value.slice(0, 10) : value);
...
function normalizeAgentContext(value) {
    ...
    return value
        .map((item) => rec(item))
        .filter((item) => CONTEXT_KINDS.has(str(item.kind)) && (str(item.id) || str(item.label)))
        .slice(0, MAX_CONTEXT_ITEMS)
        .map((item) => {
        const normalized = { kind: str(item.kind), id: str(item.id), label: str(item.label) || str(item.id) };
        if (str(item.detail))
            normalized.detail = str(item.detail);
        if (str(item.admin_code))
            normalized.admin_code = str(item.admin_code);
        return normalized;
    });
}
const KEYWORDS = {
...
function detectIntent(message) {
    ...
    const has = (key) => KEYWORDS[key].some((word) => text.includes(word));
...
function resolveTargets(situation, message, context, intent) {
    const allDistricts = seeds_js_1.seed.districts.districts;
    const allRivers = seeds_js_1.seed.rivers.rivers;
    ...
    const pushDistrict = (row) => { if (row && !districts.some((item) => str(item.district_code) === str(row.district_code)))
        districts.push(row); };
    const pushRiver = (row, explicit = false) => {
        if (!row)
            return;
        if (explicit)
            riverExplicit = true;
        if (!rivers.some((item) => str(item.river_id) === str(row.river_id)))
            rivers.push(row);
    };
    for (const item of context) {
        contextLabels.push(item.label || item.id);
        if (item.kind === 'district')
            pushDistrict(allDistricts.find((row) => str(row.district_code) === item.id || str(row.district_name) === item.label));
        if (item.kind === 'river')
            pushRiver(allRivers.find((row) => str(row.river_id) === item.id || str(row.name) === item.label), true);
        if (item.kind === 'similar_event' && item.id)
    ...
    for (const row of localDistricts.concat(allDistricts)) {
        ...
        if (name.length >= 2 && message.includes(name))
            pushDistrict(row);
    }
    ...
    if (!districts.length && typeHint && intent.district) {
        for (const row of localDistricts.filter((item) => str(item.disaster_type) === typeHint).slice(0, 3))
            pushDistrict(row);
    }
    ...
    if (!rivers.length && (intent.river || intent.threshold || intent.station))
        for (const row of localRivers)
            pushRiver(row);
    return { districts, rivers, eventIds, contextLabels, fromContext, riverExplicit, typeHint };
}
const evidenceItem = (item) => item;
function districtEvidence(district) {
    const code = str(district.district_code);
    ...
    recs(district.risk_thresholds).forEach((threshold, index) => {
        ...
        if (!str(source.doc_title) && !str(source.table))
            return;
        out.push(evidenceItem({
    ...
    recs(district.damage_events).forEach((event, index) => {
        ...
        if (!str(source.doc_title) && !str(source.passage_id))
            return;
        out.push(evidenceItem({
...
function stationDesignFlood(station) {
...
function warningStation(river) {
    ...
    if (matched)
        return { station: matched, designated: true };
    const fallback = stations.reduce((best, item) => ((stationDesignFlood(item) ?? -1) > (best ? stationDesignFlood(best) ?? -1 : -1) ? item : best), null);
    return { station: fallback, designated: false };
}
function riverEvidence(river, stations) {
    const id = str(river.river_id);
    const plan = str(river.plan_name) || str(river.name);
    const out = [];
...
async function buildAgentResponse(situation, message, context = []) {
    ...
    if (enriched && targets.contextLabels.length) {
        ...
        if (inferred.length) {
            const label = inferred.join(' · ');
            sentences.push(`질의에서 ${label}${josa(label, '을', '를')} 대상으로 인식했습니다.`);
        }
    }
    ...
    targets.districts.slice(0, 2).forEach((district, districtIndex) => {
        ...
        if (factors.length)
            parts.push(`계획자료 위험요인은 ${factors.join(' / ')}입니다.`);
        if (threshold)
        ...
        if (intent.damage || targets.fromContext) {
            ...
            if (damages.length)
                parts.push(`계획자료에 기록된 과거 참고 피해사례는 ${damages.length}건(${damages.slice(0, 2).map((item) => `${str(item.occurred)} ${str(item.event_name)}`).join(', ')})입니다.`);
        }
    ...
    if (useRiver) {
        for (const river of targets.rivers.slice(0, 2)) {
            ...
            if (flowValue !== null && station) {
                const warning = rec(station.flood_warning);
                const advisory = num(warning.advisory_m3s);
                const alert = num(warning.alert_m3s);
        ...
        if (flowValue !== null)
            extraLimitations.push('질의 유량 비교는 계획문서 기준값과의 단순 대조 참고정보이며 홍수특보 발령 판단이나 자동 조치결정이 아닙니다.');
    }
    // 유사사례·절차 축
