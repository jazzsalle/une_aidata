...
const uneRag_1 = require("../../../server/providers/uneRag");
const kmaNowcast_1 = require("../../../server/providers/kmaNowcast");
const hrfcoHydrology_1 = require("../../../server/providers/hrfcoHydrology");
const t3qMetadata_1 = require("../../../server/providers/t3qMetadata");
function status(id, name, configured, message, required, validationState, nextAction) {
    return {
        integration_id: id,
        name,
        configured,
        runtime_mode: (0, env_1.dataMode)(),
        message,
...
function GET() {
    const vworldConfigured = Boolean((0, env_1.env)('VWORLD_SERVER_API_KEY'));
    const kma = (0, kmaNowcast_1.kmaConfigured)();
    const hrfco = (0, hrfcoHydrology_1.hrfcoConfigured)();
    const rag = (0, uneRag_1.uneRagConfigured)();
    const t3qApi = false;
    const t3qMcp = false;
    const rows = [
        ...
        status('T3Q_META_ONTOLOGY', 'T3Q 구조 기반 Mock 계약', true, `현재 정책은 ${t3qMetadata_1.T3Q_RUNTIME_POLICY}이며 Event·Passage·RefDisasterEventID·T코드·lineage 구조를 UNE Mock 데이터로 구현합니다.`, [], 'fallback', '외부 API를 요청하지 않고 Mock Contract Test와 화면·검색 시나리오를 수행합니다.'),
    ...
    return (0, http_1.envelope)(rows, { provider: 'IntegrationDiagnostics', dataStatus: 'provisional' });
}
