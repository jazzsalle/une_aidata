...
const priorityAreas_1 = require("./priorityAreas");
const similarEvents_1 = require("./similarEvents");
...
async function buildAgentResponse(situation, message) {
    ...
    const similar = await (0, similarEvents_1.searchSimilarEvents)(situation, 3);
    const procedures = seeds_1.seed.procedures.procedures.filter(item => Array.isArray(item.target_admin_codes) && item.target_admin_codes.includes(situation.admin_code)).slice(0, 5);
