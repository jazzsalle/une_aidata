...
## 8. 다음 개발단계
1. Vercel Preview에서 `/evidence`의 PRE/EVENT/POST 6개 타일 로딩과 반응형 재배치를 확인한다.
2. 실제 Event 데이터가 공급되면 `event_start_at`, `event_end_at`을 기준으로 phase selection policy를 적용한다.
3. 정식 위성영상이 목표일과 일치하지 않을 경우 가장 가까운 유효 촬영일을 선택하고 목표일 편차를 `offset_days_from_target`으로 표시한다.
4. 쓰리디랩스 자료 수령 시 `ThreeDLabsSatelliteProvider`로 파일·메타데이터를 교체하되 화면 계약과 접근성 대체텍스트를 유지한다.


## 9. v0.8 구현
- `server/domain/satellitePhaseSelection.ts`: PRE/EVENT/POST 자동 선정
- `POST /api/v1/satellite-assets/select`: 목표일·유효구간·편차·선정사유 반환
- `GET /api/v1/satellite-assets/metrics`: 256×256 마스크 픽셀 상대변화 반환
- `/evidence`: 단계별 목표일 편차와 PRE 대비 픽셀 상대변화 표출
- `/report`: 선택한 영상 세트의 픽셀 상대변화를 참고근거로 반영

## 10. 실환경에서 남은 작업
1. `npm install && npm run typecheck && npm run build`
2. Vercel Preview 배포와 256×256 이미지 로딩 확인
3. VWorld 허용 도메인 등록 및 2D 지도 실검증
4. 실제 쓰리디랩스 후보영상으로 Phase Selection Contract Test
5. 공간해상도·Footprint가 제공될 때만 별도 면적계산 모듈 검토
