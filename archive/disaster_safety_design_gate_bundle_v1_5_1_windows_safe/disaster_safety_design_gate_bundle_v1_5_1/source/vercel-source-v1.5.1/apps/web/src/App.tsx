...
export default function App() {
  ...
  const [agentResponse,setAgentResponse]=useState<AgentResponse|null>(null); const [highlightedFeature,setHighlightedFeature]=useState<string|null>(null); const [error,setError]=useState<string|null>(null); const [notice,setNotice]=useState<string|null>(null);
  ...
  useEffect(()=>{if(!selected)return;setAgentResponse(null);setHighlightedFeature(null);setReportEvidence(loadReportEvidenceSelection(selected.situation_id));loadObservations(selected).then(result=>{const merged=mergeObservations(selected,result.observations);setActiveSituation(merged);if(result.meta.data_status==='actual')setNotice('기상청 초단기실황을 현재 조건에 반영했습니다.');}).catch(()=>setActiveSituation(selected));},[selected?.situation_id]);
  ...
  function onAgentResponse(response:AgentResponse){setAgentResponse(response);if(response.similar_events[0]){setSimilarEvents(response.similar_events);setSelectedEventId(response.similar_events[0].event_id);}setHighlightedFeature(response.map_actions[0]?.target_id??null);}
  ...
  return <div className="app-shell multi-page-shell">
    ...
    <main id="main-content" className={`page-main page-${route.id}`}><PageHeading title={route.title} description={route.description} status={notice}/>
      {route.id==='dashboard'?<DashboardPage situation={activeSituation??selected} priorities={priorities} procedures={procedures} events={similarEvents} selectedEventId={selectedEventId} agentResponse={agentResponse} highlightedFeature={highlightedFeature} onSituationCreated={onSituationCreated} onAgentResponse={onAgentResponse} onHighlight={setHighlightedFeature} onSelectEvent={setSelectedEventId}/>:null}
