// Phase 7 Fixture 매핑 검증 전용 — 실제 T3Q 연계·화면 활성화에 사용하지 않음. Phase 8 Shadow Test 전까지 라우트 미연결.
// data/fixtures/providers/t3q_event|t3q_risk|t3q_spatial 의 저장 payload를 계약(공통 Domain Model) 형태로
// 매핑하는 순수 함수만 제공한다. 기존 MOCK_ONLY 정책(t3qMetadata.T3Q_RUNTIME_POLICY)과
// t3qSearchConfigured()=false 에 영향이 없으며, fixture 유래 결과는 실데이터로 보이면 안 되므로
// official_data=false, data_status='mock', map_activation_allowed=false 를 강제한다(v1.1 규칙).
import type { DataStatus, PassageLineage, T3qDisasterType, T3qEventMaster, T3qPassage } from '../contracts.js';
...
function strArr(value: unknown): string[] { return arr(value).filter((item): item is string => typeof item === 'string'); }
...
function fixtureError(root: Rec): { code: string; message: string } | null {
...
const DISASTER_TYPES: readonly T3qDisasterType[] = ['FLOOD', 'TYPH', 'QUAKE', 'SLOPE', 'FIRE', 'SNOW', 'FIRE_S', 'CHEM', 'BIO'];
const EVENT_STATUSES = ['발생', '진행', '종료', '보정', '병합'] as const;
const CRISIS_LEVELS = ['LVL_01', 'LVL_02', 'LVL_03', 'LVL_04'] as const;

function disasterType(value: unknown): T3qDisasterType | null {
  return typeof value === 'string' && (DISASTER_TYPES as readonly string[]).includes(value) ? value as T3qDisasterType : null;
}
function eventStatus(value: unknown): T3qEventMaster['event_status'] | null {
  return typeof value === 'string' && (EVENT_STATUSES as readonly string[]).includes(value) ? value as T3qEventMaster['event_status'] : null;
}
function crisisLevel(value: unknown): NonNullable<T3qEventMaster['crisis_level']> | null {
  return typeof value === 'string' && (CRISIS_LEVELS as readonly string[]).includes(value) ? value as NonNullable<T3qEventMaster['crisis_level']> : null;
}

function mapEventRow(row: Rec): T3qEventMaster | null {
  ...
  if (!eventId || !type || !region || sequence === null || !status) return null;
  return {
    event_id: eventId,
    disaster_type: type,
...
function mapPassageRow(row: Rec): T3qPassage | null {
  const passageId = str(row.passage_id);
  const schemaType = str(row.schema_type);
  if (!passageId || !schemaType) return null;
  const lineageRaw = rec(row.lineage);
  const lineage: PassageLineage = {
...
export function mapT3qEventFixture(payload: unknown): { events: T3qEventMaster[]; passages: T3qPassage[]; warning?: string } {
  ...
  const events = arr(root.events).map((row) => mapEventRow(rec(row))).filter((row): row is T3qEventMaster => row !== null);
  const passages = arr(root.passages).map((row) => mapPassageRow(rec(row))).filter((row): row is T3qPassage => row !== null);
...
export const T3Q_RISK_REQUIRED_FIELDS = ['risk_id', 'hazard_codes', 'admin_code', 'district', 'risk_factors', 'thresholds', 'mitigation', 'geometry_ref', 'evidence'] as const;

export interface T3qFixtureEvidenceRef { passage_id: string; source_file?: string | null; document_page?: number | null; }
export interface T3qRiskGeometryRef { spatial_object_id: string | null; layer_id: string | null; geometry_status: string; note?: string | null; }
export interface T3qRiskKnowledge {
  risk_id: string; hazard_codes: string[]; admin_code: string; district: string;
  risk_factors: string[]; thresholds: Record<string, unknown>; mitigation: string[];
  geometry_ref: T3qRiskGeometryRef; evidence: T3qFixtureEvidenceRef[];
  data_status: DataStatus; official_data: false; is_prediction: false;
}
export interface T3qRiskFixtureResult {
  risks: T3qRiskKnowledge[];
...
function mapEvidenceRefs(value: unknown): T3qFixtureEvidenceRef[] {
  return arr(value).map(rec).flatMap((row): T3qFixtureEvidenceRef[] => {
    const passageId = str(row.passage_id);
...
export function mapT3qRiskFixture(payload: unknown): T3qRiskFixtureResult {
  const root = rec(payload);
  const base = { required_fields: T3Q_RISK_REQUIRED_FIELDS, official_data: false as const, data_status: 'mock' as const };
  const error = fixtureError(root);
  if (error) {
    return {
      ...base,
  ...
  const risks = arr(root.risks).map(rec).flatMap((row, index): T3qRiskKnowledge[] => {
    const riskId = str(row.risk_id) ?? `UNKNOWN-RISK-${index}`;
    const absent = T3Q_RISK_REQUIRED_FIELDS.filter((field) => row[field] === undefined || row[field] === null);
    if (absent.length) missing[riskId] = [...absent];
...
export const T3Q_SPATIAL_REQUIRED_FIELDS = ['spatial_object_id', 'layer_id', 'geometry', 'crs', 'properties', 'admin_code', 'hazard_codes', 'effective_at', 'evidence'] as const;

export interface T3qSpatialFixtureFeature {
  type: 'Feature';
  id: string | null;
  geometry: Rec | null;
  properties: Rec & { data_status: 'mock'; official_data: false; is_prediction: false; map_activation_allowed: false };
}
export interface T3qSpatialFixtureResult {
  feature_collection: { type: 'FeatureCollection'; crs: Rec | null; features: T3qSpatialFixtureFeature[] };
  map_activation_allowed: false;
  geometry_status: 'mock' | 'not_available';
  required_fields: readonly string[];
  missing_required_fields: Record<string, string[]>;
  official_data: false;
  data_status: 'mock';
...
export function mapT3qSpatialFixture(payload: unknown): T3qSpatialFixtureResult {
  const root = rec(payload);
  const base = {
    map_activation_allowed: false as const,
    required_fields: T3Q_SPATIAL_REQUIRED_FIELDS,
    official_data: false as const,
  ...
  const missing: Record<string, string[]> = {};
  const features = arr(collectionRaw.features).map(rec).map((row, index): T3qSpatialFixtureFeature => {
    ...
    const absent = T3Q_SPATIAL_REQUIRED_FIELDS.filter((field) => {
      if (field === 'geometry') return row.geometry === undefined || row.geometry === null;
      if (field === 'properties') return row.properties === undefined || row.properties === null;
      if (field === 'crs') return crs === null;
      return propertiesRaw[field] === undefined || propertiesRaw[field] === null;
    });
    if (absent.length) missing[featureId] = [...absent];
    return {
      type: 'Feature',
      id: str(row.id),
