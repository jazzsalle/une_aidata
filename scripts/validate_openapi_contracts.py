...
def table_routes():
    """api/index.ts 라우팅 테이블('METHOD /api/path': handler)을 파싱한다."""
...
def main():
    ...
    assert CATCH_ALL.exists(), 'api/index.ts catch-all missing'
    handlers=handler_routes(); table=table_routes(); declared,incomplete=spec_routes(spec)
    issues=[]
    issues+=diff('handler(server/routes)',handlers,'routing-table(api/index.ts)',table)
    issues+=diff('routing-table(api/index.ts)',table,'openapi(poc-backend.yaml)',declared)
